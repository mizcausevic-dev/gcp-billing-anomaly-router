...
function usage(): string {
  return [
    "Usage: gcp-billing-anomaly-router <export.json> [options]",
    "",
    "Options:",
    "  --format <json|markdown|summary>  Output format (default: summary)",
    "  --now <iso>                        Override analysis time",
    "  --stale-routing-after-hours <n>   Routing SLA before stale flag (default: 24)",
...
async function main() {
  ...
  const input = JSON.parse(fs.readFileSync(file, "utf8")) as BillingAnomalyExport;
